...
class ChatStreamingAgent( TalkingAgent):
    class CreateRoom(CyclicBehaviour):
        async def run(self):
            ...
            if msg:
                ...
                self.agent.say("Creating chat room...")
                await self.agent.session.create()
                
                plugin = await self.agent.session.attach("janus.plugin.textroom")
                ...
                if data["plugindata"]["data"]["textroom"] == "created":
                    ...
                    if "error" in data["plugindata"]["data"].keys():
                        ...
                        sucess = "Error creating chatroom!"
                else:
                    success = "Error creating chatroom - cause unknown!"
...
if __name__ == "__main__":
    ...
    name = "%s@%s" % ( CONF.chat_streaming_agent, CONF.xmpp_server )
    a = ChatStreamingAgent( name, CONF.password )
    a.start()
    a.web.start( port=CONF.chat_streaming_agent_port )
